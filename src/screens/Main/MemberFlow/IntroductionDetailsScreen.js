...
import { getIntroductionParties } from '../../../utils/introduction';
...
const IntroductionDetailsScreen = ({
  ...
  const routeIntroduction = route?.params?.introduction;
  const introductionId =
    route?.params?.introductionId ??
    routeIntroduction?._id ??
    routeIntroduction?.id;
  const {
    data: introductionDetailsResponse,
    isLoading: isLoadingDetails,
    isError: isDetailsError,
    refetch: refetchDetails,
  } = useGetIntroductionDetailsQuery(introductionId, {
    skip: !introductionId || Boolean(routeIntroduction),
  });
  const introduction =
    routeIntroduction ??
    introductionDetailsResponse?.data ??
    (introductionDetailsResponse?._id ? introductionDetailsResponse : {});
  const client = introduction.client ?? {};
  const network = introduction.networkId ?? {};
  const service = introduction.serviceId ?? {};
  const { sender, receiver, direction, counterparty } =
    getIntroductionParties(introduction, user);
  const counterpartyProfile =
    counterparty && typeof counterparty === 'object'
      ? counterparty
      : { _id: counterparty };
  const recipientId =
    counterpartyProfile.userAccountId ??
    counterpartyProfile._id ??
  ...
  useEffect(() => {
    if (introduction.status) {
      setCurrentStatus(introduction.status);
  ...
  const memberName = counterpartyProfile.name ?? 'MRN Member';
  const phone = client.phone ?? '';
  const email = client.email ?? 'Not available';
  ...
  const formattedStatus = formatIntroductionStatus(currentStatus);
  const status =
    formattedStatus === 'Not available' ? 'Active' : formattedStatus;
  ...
  const handleStatusUpdate = async () => {
    ...
    const statusIntroductionId = introduction._id ?? introduction.id;

    if (!statusIntroductionId || !nextStatus) {
      return;
    }

    try {
      const response = await updateIntroductionStatus({
        introductionId: statusIntroductionId,
        status: nextStatus,
      }).unwrap();
  ...
  const handleMessage = () => {
    ...
    const participant = getOtherParticipant(chat, user);
    const recipientProfile = getParticipantProfile(participant);

    navigation.navigate('Chat', {
      chat,
      chatId: getChatId(chat),
      participantName: recipientProfile.name || memberName,
      recipient: recipientProfile,
      recipientAccountId: getParticipantAccountId(participant),
      recipientId: recipientProfile._id ?? recipientId,
    });
  };

  if (!routeIntroduction && isLoadingDetails) {
    return (
      <Wrapper
        isScroll
  ...
  if (
    !routeIntroduction &&
    (isDetailsError || !(introduction?._id ?? introduction?.id))
  ) {
    return (
      <Wrapper
        backgroundColor={AppColors.appBgColor}
        ...
        <View style={styles.deepLinkState}>
          <AppText style={styles.deepLinkError}>
            Unable to load introduction details.
          </AppText>
          <AppButton
            title="Retry"
    ...
    <Wrapper
      ...
      <View style={styles.profileCard}>
        <View style={styles.profileTop}>
          <UserAvatar uri={counterpartyProfile.profile} style={styles.avatar} />
          <View style={styles.profileInfo}>
            <AppText style={styles.name}>{memberName}</AppText>
